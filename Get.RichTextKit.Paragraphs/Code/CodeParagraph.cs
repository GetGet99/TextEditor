...
public partial class CodeParagraph : Paragraph, ITextParagraph, IAlignableParagraph, INotifyPropertyChanged
{
    ...
    /// </summary>
    public CodeParagraph(IStyle style)
    {
    ...
    {
        _textBlock.Layout();
        internalcoloredTextBlock.Layout();
        internalcoloredTextBlock.Paint(
            canvas,
    ...
    ILanguage _Language = Languages.CSharp;
    ...
    {
        internalcoloredTextBlock.Clear();
        internalcoloredTextBlock.AddText(TextBlock);
        internalcoloredTextBlock.MaxWidth = TextBlock.MaxWidth;
        UpdateColor();
    }
    ...
    async void UpdateColor()
    {
        var success = await colorizer.FormatTextAsync(TextBlock.ToString(), _Language, EndStyle, internalcoloredTextBlock);
        if (success)
            Owner?.RequestRedraw();
    }
    TextBlock internalcoloredTextBlock = new();
}
